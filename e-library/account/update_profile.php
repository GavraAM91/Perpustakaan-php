...
if(isset($_POST['update'])){
   ...
   $image_folder = '../img-profile/'.$image;
   ...
   if(!empty($image)){
      ...
      if($image_size > 2000000){
         ...
         $update_image = $conn->prepare("UPDATE `users` SET image = ? WHERE username = ?");
   ...
   $update_profile = "UPDATE `users` SET `image`='$image',`username`='$name',
   `t_number`='$t_number',`email`='$email' WHERE username = '$user_name'";
...
<section class="update-profile-container">
   ...
   <?php
      $select_profile = mysqli_query($conn, "SELECT * FROM `users` WHERE username = '$user_name'");
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <div class="flex">
         <div class="inputBox">
            ...
            <input type="numger" name="t_number" required class="box" placeholder="enter your phone number" value="<?= $fetch_profile['t_number']; ?>">
      ...
      <div class="flex-btn">
         ...
         <a href="account.php" class="option-btn">go back</a>
      </div>
   </form>
   <?php } ?>
